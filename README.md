...
长期归档任务改用 `/volume2/quant-lab/archive/current/qyun2`，独立于冻结的退役批次；不再自动删除 45 天前的 NAS 历史。V5 脱敏快照在完成入湖解析后压缩保存；NAS 逐文件校验并生成完成回执，只有远端源与该清单再次完全一致时，才清理 qyun2 超过两天的脱敏副本。受限原包保留七天，inbox 保留两天，并且都必须先在入湖 manifest 中确认已导入。高频源清理仍必须逐批匹配校验清单，并遵守原有锁和范围检查。
