...
MARKET_ROLLUP_POLARS_MAX_THREADS="${MARKET_ROLLUP_POLARS_MAX_THREADS:-2}"
MARKET_ROLLUP_ARCHIVE_OLD_OKX_WS="${MARKET_ROLLUP_ARCHIVE_OLD_OKX_WS:-1}"
MARKET_ROLLUP_ARCHIVE_HOT_HOURS="${MARKET_ROLLUP_ARCHIVE_HOT_HOURS:-24}"
...
build_market_data_rollups() {
  ...
  if [[ "${MARKET_ROLLUP_ARCHIVE_OLD_OKX_WS}" == "1" ]]; then
    archive_args=(--archive-old-okx-public-ws --archive-hot-hours "${MARKET_ROLLUP_ARCHIVE_HOT_HOURS}")
  fi
  ...
  timeout --kill-after=30s "${MARKET_ROLLUP_TIMEOUT_SECONDS}s" \
    env POLARS_MAX_THREADS="${MARKET_ROLLUP_POLARS_MAX_THREADS}" \
...
run_small_file_maintenance() {
  ...
  timeout --kill-after=30s "${COMPACT_SMALL_FILE_MAINTENANCE_TIMEOUT_SECONDS}s" \
    ...
    --target-rows-per-file "${COMPACT_SMALL_FILE_MAINTENANCE_TARGET_ROWS}" \
