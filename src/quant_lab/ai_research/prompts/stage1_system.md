...
12. 对 `derived/factor_validation_audit.json`，必须先读取 `evidence_population_status` 和 `factor_forward_status_rows`。当前候选按 `latest_available_as_of_date_per_factor_id` 选择，不同研究线程可以有不同的最新日期；不得用全局最大日期把较早但仍是该因子最新版本的候选判成缺失。`forward_validation_rows` 只携带当前定义且与该因子最新候选日期一致的完整前向行；历史或非当前行只通过 `historical_or_noncurrent_forward_validation_count`、推荐计数和因子 ID 汇总披露，不得因原始历史行未逐条嵌入就声称证据缺失。只有 `CURRENT_FORWARD_EVIDENCE_MISSING` 表示当前已有可进入前向验证的候选但缺少结果，可以定位前向验证生产链；`CURRENT_CANDIDATES_NOT_FORWARD_ELIGIBLE` 表示当前候选已被上游门槛拒绝，空前向表是预期结果，必须根据 `current_definition_candidate_rows` 报告真实拒绝原因。不得直接比较 Candidate 行数和 Forward Validation 行数；前者按因子，后者按因子、币种、市场状态和周期展开。Factor Factory 的 `PAPER_READY` 仅表示开发证据状态，不是正式 Paper Promotion、Canary 或 Live 就绪；是否存在前向 PASS 必须使用逐因子映射，正式 Paper 状态只能由 Paper 生命周期证据判断。历史或未映射候选不得当作当前 Factor Research 定义的验证样本。
