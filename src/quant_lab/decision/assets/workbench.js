...
function renderForward() {
  ...
  $("forward-summary").innerHTML = `<dl class="summary-strip">${items.map(([k,v]) => `<div class="summary-item"><dt>${esc(k)}</dt><dd class="number">${num(v, 0)}</dd></div>`).join("")}</dl>
    <p class="flat-note">实验：${esc(f?.experiment || "旧版汇总，范围未绑定")} · 策略：${esc(f?.strategy_version || "未声明")} · 成本版本：${(f?.cost_versions || []).map(esc).join(" / ") || "未声明"}。<br>筛选区间：${time(f?.published_from, true)} — ${time(f?.published_until, true)}，不代表实验运行时长。旧口径非重叠登记 ${num(f?.non_overlapping_opportunities, 0)}（按最长时域，包含未成熟），其余价格观察 ${num(f?.overlapping_price_observations, 0)}。</p>
    <p class="flat-note">前向起点：${time(f?.started_at, true)}。同一机会的 4h 与 24h 是两个观察维度，不是两次独立交易。到期但缺行情的窗口：${num(f?.missing_label_observations, 0)}。<br>本页价格标签不包含 V5 消费回执和同资金账户收益；账户增益需在 V5 模拟账户对照中另行核验。</p>`;
  $("forward-summary").innerHTML += coverage ? `<h3>登记连续性</h3><p class="notice ${coverage.status === "GAPS" ? "warning" : ""}">${coverage.status === "GAPS" ? "存在未登记小时，缺标签为零不代表连续采满。" : coverage.status === "NOT_STARTED" ? "当前版本尚无及时发布的前向登记。" : "截至已到截止时间的小时，登记完整。"}历史缺口保留，不以事后回放补成前向记录。</p><p class="flat-note">从首个登记入场时点 ${time(coverage.first_entry_at, true)} 起，统计至 ${time(coverage.due_through_entry_at, true)}；每币每小时应同时有 4h 和 24h 登记。尚未到截止时间的下一小时单独列出。发布时间距行情闭合超过 15 分钟列为延迟。</p><div class="table-scroll"><table class="simple-table"><thead><tr><th>币种</th><th>应登记小时</th><th>完整登记</th><th>缺口</th><th>覆盖率</th><th>未到截止</th><th>延迟小时</th></tr></thead><tbody>${coverage.by_symbol.map(r => `<tr><td>${esc(r.symbol)}</td><td>${num(r.expected_hours,0)}</td><td>${num(r.registered_hours,0)}</td><td>${num(r.missing_hours,0)}${r.partial_horizon_hours ? `（含 ${num(r.partial_horizon_hours,0)} 个时域不全）` : ""}</td><td>${r.coverage_fraction == null ? "—" : `${num(r.coverage_fraction * 100,2)}%`}</td><td>${num(r.pending_deadline_hours,0)}</td><td>${num(r.late_publication_hours,0)}</td></tr>`).join("")}</tbody></table></div><details><summary>缺口时段与延迟</summary>${coverage.by_symbol.map(r => `<p class="flat-note">${esc(r.symbol)} · 最大发布延迟 ${num(r.maximum_publication_delay_seconds,0)} 秒<br>${r.gaps.map(g => `${time(g.first_entry_at,true)} — ${time(g.last_entry_at,true)}：${num(g.hours,0)} 小时`).join("<br>") || "无已到期登记缺口"}${r.omitted_gap_ranges ? `<br>另有 ${num(r.omitted_gap_ranges,0)} 段较早缺口，已计入总数。` : ""}</p>`).join("")}</details>` : `<p class="notice warning">当前结果未携带登记连续性统计，不能将缺标签数当作采样覆盖率。</p>`;
  ...
  $("forward-groups").innerHTML = `<h3>全部成熟价格标签（窗口可重叠）</h3><p class="flat-note">DEFER 后资产涨跌仅是价格标签，不代表过滤策略赚到或避免了这些收益。</p>` + $("forward-groups").innerHTML;
  const selected = f?.mature_non_overlapping_by_group || [];
  if (selected.length) $("forward-groups").innerHTML += `<h3>按币种和时域去重的成熟标签</h3><p class="flat-note">先按登记时间选取最早且互不重叠的窗口，再观察结果；未成熟及缺行情不会被替换。四币存在相关性，不能相加称为独立交易样本。成本场景收益仍不是账户收益。</p><div class="table-scroll"><table class="simple-table"><thead><tr><th>币种</th><th>时域</th><th>动作</th><th>已选窗口</th><th>成熟</th><th>等待 / 缺标签</th><th>净均值（有成本样本）</th></tr></thead><tbody>${selected.map(g => `<tr><td>${esc(g.symbol)}</td><td>${esc(g.horizon_hours)}h</td><td>${esc(LABELS[g.action] || g.action)}</td><td>${num(g.selected_observations,0)}</td><td>${num(g.matured_observations,0)}</td><td>${num(g.waiting_observations,0)} / ${num(g.missing_label_observations,0)}</td><td>${bps(g.net_mean_bps)}（${num(g.net_observations,0)}）</td></tr>`).join("")}</tbody></table></div>`;
}
