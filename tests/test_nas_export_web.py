...
import quant_lab.export_plane.cloud_index as cloud_index
...
def test_nas_web_status_separates_requested_date_from_previous_available_pack(
    tmp_path: Path,
    monkeypatch,
) -> None:
    secret = tmp_path / "download.key"
    ...
    monkeypatch.setenv("QUANT_LAB_NAS_DOWNLOAD_SIGNING_SECRET_FILE", str(secret))
    previous = {
        "pack_name": "quant_lab_expert_pack_2026-07-15_previous.zip",
        "download_url": "http://nas.invalid:8788/download/previous",
        "pack_size_bytes": 42,
        "accepted_at": "2026-07-15T01:00:00Z",
    }
    monkeypatch.setattr(
        cloud_index,
        "export_plane_status",
        lambda *_args, **_kwargs: {
            "state": "missing_requested_date",
            "task": None,
            "request": None,
            "latest_pack": previous,
            "requested_date_pack": None,
            "available_pack": previous,
            "packs": [previous],
            "download_ready": False,
            "available_download_ready": True,
        },
    )

    payload = api_main._web_v2_nas_expert_pack_status(export_date=date(2026, 7, 16))

    assert payload["state"] == "missing_requested_date"
    assert payload["download_ready"] is False
    assert payload["requested_date_pack_name"] is None
    assert payload["available_pack_name"] == previous["pack_name"]
    assert payload["available_download_ready"] is True
